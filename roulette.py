...
def rouletteRDA(app):
    ...
    drawLabel("5 XRP per spin.", app.width / 2, 70, fill='white')
    drawLabel(f"Balance: {app.balance} XRP", app.width / 2, 100, bold=True, size=30, fill='white')
...
def checkWin(app, goal, state):
    ...
    if state == "RED":
        ...
        if goal in nums:
            win = True
            app.balance += 5 * int(float(app.currOdds[1:]))

    app.victory = win

def rouletteOMP(app, x, y):
