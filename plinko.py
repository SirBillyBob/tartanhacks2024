...
class Plinko:
    ...
    def onKeyPress(self, app, key):
        if key == '1' and self.balance >= 1:
            ...
            self.loading = self.app.loading = True

    def onStep(self, app):
        if self.loading:
            self.loadingTimer += 1
            if self.loadingTimer >= 5:
    ...
    def redrawAll(self, app):
        if self.loading:
            drawLabel("Making transactions...", self.width / 2, self.height / 2, size=30, bold=True, fill='lightSlateGray')
        else:
            for peg in self.pegs:
                peg.draw()
